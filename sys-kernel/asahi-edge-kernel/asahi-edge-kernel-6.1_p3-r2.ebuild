...
src_prepare() {
	...
	sed -i -e 's/rustc_allocator_nounwind/rustc_nounwind/g' \
		rust/alloc/alloc.rs || die "Failed to fix rust/alloc/alloc.rs"
	sed -i -e 's/const Unpin/Unpin/' \
		rust/alloc/boxed.rs || die "Failed to fix rust/alloc/boxed.rs"
	sed -i -e '/^pub unsafe trait RawDeviceId/i #[const_trait]' \
		rust/kernel/driver.rs || die "Failed to fix rust/kernel/driver.rs"
